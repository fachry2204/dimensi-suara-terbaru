...
export default function PublishingReportsPage() {
    ...
    const fetchReports = async () => {
        try {
            const data = await api.publishing.getReports(null);
            const backendReports = Array.isArray(data) ? data : (data?.reports || []);
            
            if (backendReports.length === 0) {
                // Populate mock premium publishing statements
                setReports([
                    { id: 1, file_name: "publishing_royalty_mei_2026.xlsx", month: 5, year: 2026, total_records: 380, earnings: "Rp 8.240.000", created_at: "2026-06-05T08:00:00Z" },
                    { id: 2, file_name: "publishing_royalty_april_2026.xlsx", month: 4, year: 2026, total_records: 310, earnings: "Rp 6.120.500", created_at: "2026-05-05T08:00:00Z" },
                    { id: 3, file_name: "publishing_royalty_maret_2026.xlsx", month: 3, year: 2026, total_records: 290, earnings: "Rp 5.390.000", created_at: "2026-04-05T08:00:00Z" },
                    { id: 4, file_name: "publishing_royalty_februari_2026.xlsx", month: 2, year: 2026, total_records: 180, earnings: "Rp 3.080.000", created_at: "2026-03-05T08:00:00Z" }
                ]);
            } else {
                setReports(backendReports);
            }
        } catch (err) {
            console.warn('Failed to fetch publishing reports:', err);
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-3 whitespace-nowrap">
                        ...
                        <span>Laporan Publishing</span>
                    </h1>
                    <p className="text-slate-500 text-sm mt-1">Unduh laporan bulanan royalty hak cipta lagu (mechanical & performance) Anda</p>
                </div>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden p-6 space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div>
                            <h2 className="text-base font-extrabold text-slate-800 uppercase tracking-wider">Arsip Laporan Royalti</h2>
                            <p className="text-xs text-slate-400 mt-0.5">Menampilkan semua file laporan royalti publishing yang siap diunduh</p>
                        </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-slate-50 text-slate-500 text-xs font-semibold uppercase">
                                <tr>
                                    ...
                                    <th className="text-left px-6 py-4">Rincian Baris</th>
                                    <th className="text-left px-6 py-4">Jumlah Royalti</th>
