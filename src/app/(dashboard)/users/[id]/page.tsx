...
export default function UserDetailPage() {
  ...
  const [userRole] = useState(typeof window !== 'undefined' ? (localStorage.getItem('cms_role') || '') : '');
  const [currentUser, setCurrentUser] = useState<User | null>(null);
  const [user, setUser] = useState<User | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [statusDraft, setStatusDraft] = useState<User['status'] | null>(null);
  const [contractStatusDraft, setContractStatusDraft] = useState<string>('Not Generated');
  ...
  const isImpersonatingActive = typeof window !== 'undefined' ? (localStorage.getItem('is_impersonating') === 'true') : false;
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const updatedData: Partial<User> & Record<string, any> = { ...editFormData };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const merged: User = {
        ...user,
        ...res.user,
        contract_status: res.user.contract_status ?? user.contract_status,
  ...
  if (isLoading) {
    return (
      <div className="p-8 w-full max-w-none min-h-screen">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-slate-600">Memuat data...</div>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="p-8 w-full max-w-none min-h-screen">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-slate-600">User tidak ditemukan</div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 animate-fade-in">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-3">
            <h3 className="text-base font-semibold text-slate-800">Profile Lengkap</h3>
          ...
          <button onClick={() => router.push('/users')} className="text-slate-400 hover:text-slate-600">
            <XCircle size={24} />
          </button>
        </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {user.ktp_doc_path && (
                  <div className="border border-slate-200 rounded-xl p-3">
                    <div className="text-xs font-medium mb-2 text-slate-700">KTP</div>
                ...
                {user.npwp_doc_path && (
                  <div className="border border-slate-200 rounded-xl p-3">
                    <div className="text-xs font-medium mb-2 text-slate-700">NPWP</div>
                ...
                {user.signature_doc_path && (
                  <div className="border border-slate-200 rounded-xl p-3">
                    <div className="text-xs font-medium mb-2 text-slate-700">File Tandatangan</div>
                ...
                {(user.account_type === 'COMPANY') && user.nib_doc_path && (
                  <div className="border border-slate-200 rounded-xl p-3">
                    <div className="text-xs font-medium mb-2 text-slate-700">NIB</div>
                ...
                {(user.account_type === 'COMPANY') && user.kemenkumham_doc_path && (
                  <div className="border border-slate-200 rounded-xl p-3">
                    <div className="text-xs font-medium mb-2 text-slate-700">Kemenkumham</div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-scale-in">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-medium uppercase text-slate-700">Nama</label>
                  <input
                    type="text"
                    value={editFormData.full_name || ''}
                    onChange={(e) => setEditFormData({ ...editFormData, full_name: e.target.value })}
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-slate-700"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-medium uppercase text-slate-700">Email</label>
                  <input
                    type="email"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium uppercase text-slate-700">Password (Optional)</label>
                  <input
                    type="password"
