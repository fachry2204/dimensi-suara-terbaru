...
const filePath = path.join(__dirname, '../components/ChunkUploader.tsx');
...
export const ChunkUploader: React.FC<ChunkUploaderProps> = ({ 
  ...
  const uploadFile = async (selectedFile: File) => {
    ...
    try {
      ...
      // 2. Upload chunks sequentially (or concurrent with limit)
  ...
  return (
    <div className="border border-slate-200 rounded p-4 bg-white relative">
      ...
      {status === 'SUCCESS' && (
        <div className="w-full border border-green-200 rounded-lg p-4 bg-green-50 flex items-center justify-between">
          <div className="flex items-center">
            <CheckCircle className="text-green-500 mr-2" size={20} />
            <div className="truncate">
              <p className="text-sm font-medium text-green-800 truncate">{file?.name}</p>
              <p className="text-xs text-green-600 mt-0.5">Berhasil diunggah dan diverifikasi</p>
            </div>
          </div>
          <button 
            type="button"
            onClick={reset}
            className="p-1.5 bg-white text-slate-500 rounded-md shadow-sm border border-slate-200 hover:text-red-500 hover:border-red-200 transition-colors"
          >
            <X size={16} />
          </button>
        </div>
      ...
      {status === 'ERROR' && (
        <div className="w-full border border-red-200 rounded-lg p-4 bg-red-50 flex flex-col items-start relative">
          ...
          <button 
            ...
            className="mt-3 text-xs font-medium bg-white px-3 py-1.5 border border-red-200 rounded text-red-600 hover:bg-red-50"
          >
            Coba Lagi
          </button>
        </div>
...
console.log('ChunkUploader component created.');
